...
const registerUser = async (user, additionalData = {}) => {
  ...
  if (error) {
    ...
    const { password: _p, confirm_password: _cp, ...safeUser } = user;
    logger.info(
      'Route: register - Validation Error',
      { name: 'Request params:', value: safeUser },
  ...
  const { email, password, name, username, provider, termsAccepted } = user;

  if (termsAccepted !== true) {
    return { status: 400, message: 'You must accept the terms of service to register.' };
  }
  ...
  try {
    ...
    if (existingUser) {
      const { password: _pw, confirm_password: _cpw, ...safeParams } = user;
      logger.info(
        'Register User - Email in use',
        { name: 'Request params:', value: safeParams },
